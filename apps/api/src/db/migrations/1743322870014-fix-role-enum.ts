...
export class FixRoleEnum1743322870014 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (enumExists[0]?.enum_exists) {
      ...
      await queryRunner.query(
        `CREATE TYPE users_role_enum_new AS ENUM ('ADMIN')`
      );

      // Drop the default constraint first (required when changing column type)
      await queryRunner.query(`
                ALTER TABLE "users" 
      ...
      await queryRunner.query(`
                ALTER TABLE "users" 
                ALTER COLUMN "role" SET DEFAULT 'ADMIN'::users_role_enum_new
            `);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    if (enumExists[0]?.enum_exists) {
      ...
      await queryRunner.query(
        `CREATE TYPE users_role_enum_new AS ENUM ('ADMIN', 'DOCTOR', 'PATIENT')`
      );
